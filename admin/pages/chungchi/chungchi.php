                ...
                <div class="col-lg-12">
                    <div class="panel panel-default">
                        <div class="panel-heading">
                            quản lý chứng chỉ <a href="themCC.php"><button class="login100-form-btn pull-right" name="btnthem" >thêm</button></a>
                        </div>
                        <a href="#">
                        <div class="panel-body">
                            <table width="100%" class="table table-striped table-bordered table-hover" id="dataTables-example">
                                ...
                                <tbody>
                                    <?php
                                    $chungchi = ChungChi();
                                    while($row_chungchi = mysqli_fetch_array($chungchi))
                                    {
                                        ob_start();
                                ?>
                                    <tr class="odd gradeX">
                                        <td>{ma}</td>
                                        <td>{ten}</td>
                                        <td><a href="suaCC.php?maCC={ma}&tenCC={ten}"><button class="login100-form-btn" name="btnsua" >sửa</button></a> <a href="xoaCC.php?maCC={ma}""><button class="login100-form-btn" name="btnxoa" >xóa</button></a></td>
                                    </tr>
                                    <?php
                                      $s = ob_get_clean();
                                      $s = str_replace("{ma}",$row_chungchi['maCC'],$s);
                                      $s = str_replace("{ten}",$row_chungchi['tenCC'],$s);
                                      echo $s;
                                    }
                                    ?>
                                </tbody>
